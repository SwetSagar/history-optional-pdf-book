---
name: Patne
categories: [Palaeolithic, Mesolithic]
state: Maharashtra
coords: [16.4272, 73.5178]
coords_confidence: high
coords_from: ""
coords_provisional: true
dot_px: [206.0, 438.4]
images: [Individual Map sites History Optional/1 Palaeolithic/Patne.jpg, Individual Map sites History Optional/2 Mesolithic/Patne.jpg]
status: sourced
sources: [upinder2008]
locked: []
sources_proposed: ["upinder2008#2. Palaeolithic and Mesolithic@33:strong", "upinder2008#2. Palaeolithic and Mesolithic@32:strong", "upinder2008#2. Palaeolithic and Mesolithic@34:strong", "upinder2008#2. Palaeolithic and Mesolithic@27:weak"]
---

A site in the Tapti valley, Maharashtra, showing a stratigraphic sequence of Middle Palaeolithic, Upper Palaeolithic and Mesolithic tools.

- A gradual reduction in tool size can be traced from the late Pleistocene, with microliths of that age.
- An ostrich eggshell engraved with a criss-cross pattern was recovered, together with beads made from the shell.
